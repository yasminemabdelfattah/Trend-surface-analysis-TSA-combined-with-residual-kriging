...
STFDF = as(m, "STFDF")
library(lubridate)
yearaverage <- aggregate(rainanom ~Year, data=STFDF,FUN=mean)
yearaveragets <- ts(yearaverage$rainanom, start=1998, end=2126, frequency=1) 
View(yearaveragets)
plot(yearaveragets, col="darkgrey")
lines(lowess(time(yearaveragets), yearaveragets), col="blue", lwd=2)
acf(yearaveragets)
pacf(yearaveragets)
dev.off()
library(rkt)
rkt(yearaverage$Year, yearaverage$rainanom)
mkttest(yearaverage$rainanom)

monthlyaverage <- aggregate(rainanom ~Year+Month, data=STFDF,FUN=mean)
monthlyaveragets <- ts(monthlyaverage$rainanom, start=c(1998,1), end=c(2126,1), frequency=12) 
View(monthlyaveragets)
plot(monthlyaveragets, col="darkgrey")
lines(lowess(time(monthlyaveragets), monthlyaveragets), col="blue", lwd=2)
acf(monthlyaveragets)
pacf(monthlyaveragets)
library(rkt)
rkt(monthlyaverage$Year, monthlyaverage$rainanom, monthlyaverage$Month, correct = TRUE)
mkttest(monthlyaverage$rainanom)
tfpwmk(monthlyaverage$rainanom)
library(Kendall)
SeasonalMannKendall(monthlyaveragets)
...
STFDF = as(m, "STFDF")
kirmetaverage<- aggregate(rainanom ~Year+Month, data=STFDF,FUN=mean)
kirmetaveragets <- ts(kirmetaverage$rainanom, start=c(1998,1), end=c(2126,1), frequency=12) 
plot(kirmetaveragets, col="darkgrey")
lines(lowess(time(kirmetaveragets), kirmetaveragets), col="blue", lwd=2)
acf(kirmetaveragets)
pacf(kirmetaveragets)
library(rkt)
rkt((kirmetaverage$Year), kirmetaverage$rainanom, kirmetaverage$Month)
tfpwmk(kirmetaverage$rainanom)
...
STFDF = as(m, "STFDF")
belgaverage<- aggregate(rainanom ~Year+Month, data=STFDF,FUN=mean)
belgaveragets <- ts(belgaverage$rainanom, start=c(1998,1), end=c(2126,1), frequency=12) 
plot(belgaveragets, col="darkgrey")
lines(lowess(time(belgaveragets), belgaveragets), col="blue", lwd=2)
acf(belgaveragets)
pacf(belgaveragets)
library(rkt)
rkt((belgaverage$Year), belgaverage$rainanom, belgaverage$Month)
tfpwmk(belgaverage$rainanom)
...
STFDF = as(m, "STFDF")
begaaverage<- aggregate(rainanom ~Year+Month, data=STFDF,FUN=mean)
begaaveragets <- ts(begaaverage$rainanom, start=c(1998,1), end=c(2126,1), frequency=12) 
plot(begaaveragets, col="darkgrey")
lines(lowess(time(begaaveragets), begaaveragets), col="blue", lwd=2)
acf(begaaveragets)
pacf(begaaveragets)
library(rkt)
rkt((begaaverage$Year), begaaverage$rainanom, begaaverage$Month)
...
STFDF = as(m, "STFDF")
Janaverage<- aggregate(rainanom ~Year+Month, data=STFDF,FUN=mean)
Janaveragets <- ts(Janaverage$rainanom, start=c(1998,1), end=c(2126,1), frequency=12) 
plot(Janaveragets, col="darkgrey")
lines(lowess(time(Janaveragets), Janaveragets), col="blue", lwd=2)
acf(Janaveragets)
pacf(Janaveragets)
library(rkt)
rkt(Janaverage$Year, Janaverage$rainanom)
...
STFDF = as(m, "STFDF")
Febaverage<- aggregate(rainanom ~Year+Month, data=STFDF,FUN=mean)
library(rkt)
rkt(Febaverage$Year, Febaverage$rainanom)
Febaveragets <- ts(Febaverage$rainanom, start=1998, end=2126, frequency=1) 
View(Febaveragets)
plot(Febaveragets, col="darkgrey")
lines(lowess(time(Febaveragets), Febaveragets), col="blue", lwd=2)
acf(Febaveragets)
pacf(Febaveragets)

Mar=subset(TRMMENSO, Month ==3)
library(spacetime)
m = stConstruct(Mar, c("lon", "lat"), "date", crs = CRS("+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0"))
STFDF = as(m, "STFDF")
Maraverage<- aggregate(rainanom ~Year+Month, data=STFDF,FUN=mean)
library(rkt)
rkt(Maraverage$Year, Maraverage$rainanom)

April=subset(TRMMENSO, Month ==12)
library(spacetime)
m = stConstruct(April, c("lon", "lat"), "date", crs = CRS("+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0"))
STFDF = as(m, "STFDF")
Aprilaverage<- aggregate(rainanom ~Year+Month, data=STFDF,FUN=mean)
library(rkt)
rkt(Aprilaverage$Year, Aprilaverage$rainanom)
